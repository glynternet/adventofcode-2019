...
func part01(input []int64) {
	...
	go computer{memory: memory(input).copy()}.runUntilHalt(1, outCh)
	var count int
...
func part02(input []int64) {
	outCh := make(chan int64)
	go computer{memory: memory(input).copy()}.runUntilHalt(5, outCh)
	var count int
...
func (c computer) runUntilHalt(input int64, output chan<- int64) {
